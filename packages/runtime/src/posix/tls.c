...
static pthread_mutex_t tls_hs_result_mu = PTHREAD_MUTEX_INITIALIZER;
...
static void drain_hs_results(void) {
  for (;;) {
    pthread_mutex_lock(&tls_hs_result_mu);
    SnTlsHsResult *r = tls_hs_results;
    if (r != NULL) {
      tls_hs_results = r->next;
    }
    pthread_mutex_unlock(&tls_hs_result_mu);
    if (r == NULL) {
      break;
    }
...
static void *tls_hs_one_shot(void *arg) {
...
static void enqueue_handshake(SnTlsConn *conn, SnFuture *fut, int is_accept) {
  ...
  job->next = NULL;
  pthread_t th;
  if (pthread_create(&th, NULL, tls_hs_one_shot, job) != 0) {
    abort();
  }
  pthread_detach(th);
}
...
static int tcp_fd_from_handle(int64_t tcp_handle) {
...
static void tls_after_tcp(SnFuture *tcp_fut) {
  ...
  int fd = tcp_fd_from_handle(tcp_handle);
  if (fd < 0) {
