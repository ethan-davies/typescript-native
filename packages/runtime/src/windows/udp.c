...
static void recv_cb(void *userdata, int events) {
  ...
  int n = recvfrom((SOCKET)req->sock->fd, (char *)buf, maxb, 0, (struct sockaddr *)&peer, &peerlen);
...
static void send_cb(void *userdata, int events) {
  ...
  int n = sendto((SOCKET)req->sock->fd, (const char *)data, (int)len, 0, (struct sockaddr *)&req->addr,
                 req->addrlen);
  sn_reactor_del_fd(req->sock->fd);
  if (n < 0) {
    if (would_block()) {
...
void *sn_udp_send(int64_t socket_handle, int64_t bytes_handle, const char *host, int32_t port) {
  ...
  int n = sendto((SOCKET)sock->fd, (const char *)data, (int)len, 0, res->ai_addr, (int)res->ai_addrlen);
  if (n >= 0) {
    freeaddrinfo(res);
  ...
  req->addrlen = (int)res->ai_addrlen;
  req->sent = 0;
  freeaddrinfo(res);
